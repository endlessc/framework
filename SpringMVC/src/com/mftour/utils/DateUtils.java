...
public class DateUtils {
	...
	public static SimpleDateFormat dateFormat = new SimpleDateFormat(
			"yyyy-MM-dd HH:mm:ss");
	public static SimpleDateFormat dateFormatCHN = new SimpleDateFormat(
			"yyyy年MM月dd日");
	public static SimpleDateFormat yearFormat = new SimpleDateFormat("yyyy");
	
	public static TimeZone DEFAULT_TIME_ZONE = TimeZone
	...
	public static int daysFrom(java.sql.Timestamp time) {
		int offset = Calendar.getInstance(DEFAULT_TIME_ZONE).get(
				Calendar.ZONE_OFFSET);
		return (int) ((System.currentTimeMillis() + offset)
				/ TimeUnit.DAYS.toMillis(1) - (time.getTime() + offset)
	...
	public static long dayStart(int afterDays) {
		...
		long dayStart = cal.getTimeInMillis() / 1000 * 1000
				+ TimeUnit.DAYS.toMillis(afterDays);
		return dayStart;
	}
	...
	public static boolean isSameMonth(Timestamp time) {
		...
		return last.get(Calendar.MONTH) == now.get(Calendar.MONTH)
	...
	public static long monthEnd() {
		...
		System.out.println("nextMonthStart:"
				+ getDateTimeString(new Timestamp(nextMonthStart)));
	...
	public static long getLatestPhaseStart(int delay, long curTime) {
		...
		if (delay == 7 * 24) {
			...
			return ((curTime + TimeUnit.DAYS.toMillis(3) + zoneOffset) / TimeUnit.HOURS
					.toMillis(delay))
					* TimeUnit.HOURS.toMillis(delay)
					- TimeUnit.DAYS.toMillis(3) - zoneOffset;
		} else if (delay == 24) {
			return ((curTime + zoneOffset) / TimeUnit.HOURS.toMillis(delay))
					* TimeUnit.HOURS.toMillis(delay) - zoneOffset;
		} else if (delay > 0 && delay < 24) {
			return (curTime / TimeUnit.HOURS.toMillis(delay))
					* TimeUnit.HOURS.toMillis(delay);
		} else {
	...
	public static int phaseOf1970(int delay, long curTime) {
		...
		if (delay == 7 * 24) {
			// 针对星期进行修正，1970.01.01是周四
			return (int) ((getLatestPhaseStart(delay, curTime)
					- TimeUnit.DAYS.toMillis(3) - zoneOffset) / TimeUnit.HOURS
					.toMillis(delay));
		} else if (delay == 24) {
			return (int) (getLatestPhaseStart(delay, curTime) / TimeUnit.HOURS
					.toMillis(delay));
		} else if (delay > 0 && delay < 24) {
			return (int) (getLatestPhaseStart(delay, curTime) / TimeUnit.HOURS
					.toMillis(delay));
	...
	public static int numDaysFrom(Timestamp ts1, Timestamp ts2) {
		// ts1 is smaller
		// ts2 is bigger
		return calculateJulianDayNumberAtMidnight(ts2)
				- calculateJulianDayNumberAtMidnight(ts1);
	}
	...
	public static boolean judgeSettleDate(Date fromDate, Date toDate, int period) {
		...
		//打印出来的是今天结算日期
		System.out.println("下次结算开始日期：" + cal_from.get(Calendar.YEAR) + "-" + cal_from.get(Calendar.MONTH) + "-" + cal_from.get(Calendar.DAY_OF_MONTH));
		//打印出来的是上次结算日期
		System.out.println("上次结算日期：" + cal_to.get(Calendar.YEAR) + "-" + cal_to.get(Calendar.MONTH) + "-" + cal_to.get(Calendar.DAY_OF_MONTH));
	...
	public static Date addDateTime(String date,int day){
		return addDateTime(string2Date(date), day);
	}
     ...
     */
	  ...
	  */
		...
		private static final java.text.SimpleDateFormat dateFormatG = new java.text.SimpleDateFormat(
				DATE_PATTERN);
		...
		public static int getMonthDays(Date date,int num){
			...
			for(int i=1;i<=num;i++){
				 cal.add(Calendar.MONTH,1);
				 //下月剩余天数
				 int dayNextMonth=cal.getActualMaximum(Calendar.DAY_OF_MONTH);	
				 total+=dayNextMonth;
			}
			return total;
		}
		/**
		 * 获得两个日期之间（包含）所有的日期集合
		 * 例如  传入  2015-01-01  2015-01-05  ，返回list   2015-01-01,2015-01-02,2015-01-03,2015-01-04,2015-01-05
		 * @param startTime 开始时间，格式yyyy-MM-dd 
		 * @param endTime 结束时间，格式yyyy-MM-dd 
		 * 	fromat   返回对应格式
		 * @return
		 */
		public static List<String> getDaysByStartTimeToEndTime(String startTime,String endTime,String format){
			/*if(StringUtils.isEmpty(startTime)||StringUtils.isEmpty(endTime)) return null;*/
			if(parseDate(startTime, DATE_PATTERN).getTime() > parseDate(endTime, DATE_PATTERN).getTime()) return null;
			List<String> list = new ArrayList<String>();
			if(formatDate( format,parseDate(startTime, format)).equals(formatDate( format,parseDate(endTime, format)))){
				if (!list.contains(formatDate( format,parseDate(startTime, format)))) 
				list.add(formatDate( format,parseDate(startTime, format)));
			}
			while (parseDate(startTime, DATE_PATTERN).getTime() <= parseDate(endTime, DATE_PATTERN).getTime()) {
				if (!list.contains(formatDate( format,parseDate(startTime, format)))) 
					list.add(formatDate( format,parseDate(startTime, format)));
				startTime = format( getAppointDate(parseDate(startTime, DATE_PATTERN), 1));
			}
			return list;
		}
		...
		public static void main(String[] args) {
			//System.out.println(getMonthDays(new Date(),1));
			List<String> list = new ArrayList<String>();
			list = getDaysByStartTimeToEndTime("2015-01-01", "2015-05-03","yyyy-MM-dd");
